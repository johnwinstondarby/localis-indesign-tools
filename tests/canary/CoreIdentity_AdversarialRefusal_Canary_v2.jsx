...
/*
core/identity modular adversarial refusal canary v2.0.0
Contract candidate: NORMALFIX_OBJECT_IDENTITY_CONTRACT_v0_3.md
Contract version exercised by this canary: 0.3.0
...
The canary:
  ...
  4. runs sixteen adversarial refusal cases;
...
Cases:
  ...
  IR13 KinsokuTable semantic-name mismatch refuses
  IR14 KinsokuTable stored-ID conflict refuses
  IR15 MojikumiTable semantic-name mismatch refuses
  IR16 MojikumiTable stored-ID conflict refuses
*/

(function () {
    var CANARY_VERSION = "2.0.0";
    ...
    function exactNamedItem(collection, name) {
        var items = collectionItems(collection);
        var i;
        for (i = 0; i < items.length; i++) {
            if (stringProp(items[i], "name") === name) {
                return items[i];
            }
        }
        return null;
    ...
    var kinsokuA, kinsokuB, mojikumiA, mojikumiB;
    ...
    try {
        ...
        kinsokuA = doc.kinsokuTables.add(
            "NF Identity Kinsoku A",
            {
                cantBeginLineChars:"、。）》」』】",
                cantEndLineChars:"（《「『【",
        ...
        kinsokuB = doc.kinsokuTables.add(
            "NF Identity Kinsoku B",
            {
                cantBeginLineChars:"、。）》」』】",
                cantEndLineChars:"（《「『【",
                hangingPunctuationChars:"、。",
        ...
        mojikumiA = doc.mojikumiTables.add(
            "NF Identity Mojikumi A",
            {basedOnMojikumiSet:MojikumiTableDefaults.LINE_END_ALL_ONE_EM_ENUM}
        );
        mojikumiB = doc.mojikumiTables.add(
            "NF Identity Mojikumi B",
            {basedOnMojikumiSet:MojikumiTableDefaults.LINE_END_ALL_ONE_EM_ENUM}
        );

        require(valid(kinsokuA) && valid(kinsokuB),
            "KinsokuTable fixture creation failed.");
        require(valid(mojikumiA) && valid(mojikumiB),
            "MojikumiTable fixture creation failed.");
        ...
        /*
         * Kinsoku/Mojikumi fixture reacquisition uses exact collection
         * enumeration rather than collection index, built-in roster position,
         * translated names, or resource creation.
         */
        kinsokuA = exactNamedItem(doc.kinsokuTables, "NF Identity Kinsoku A");
        kinsokuB = exactNamedItem(doc.kinsokuTables, "NF Identity Kinsoku B");
        mojikumiA = exactNamedItem(doc.mojikumiTables, "NF Identity Mojikumi A");
        mojikumiB = exactNamedItem(doc.mojikumiTables, "NF Identity Mojikumi B");
        ...
        require(valid(kinsokuA) && valid(kinsokuB),
            "KinsokuTable fixture reacquire failed.");
        require(valid(mojikumiA) && valid(mojikumiB),
            "MojikumiTable fixture reacquire failed.");
        ...
        runCase(
            ...
            function () {
                var i;
                var kState = serializeIdentity(kinsokuA);
                var mState = serializeIdentity(mojikumiA);

                require(kState.ok, "KinsokuTable serialization failed for plain-data audit.");
                require(mState.ok, "MojikumiTable serialization failed for plain-data audit.");
                allStates.push(kState.state);
                allStates.push(mState.state);

                require(allStates.length >= 12,
        ...
        runCase(
            "IR13",
            "KinsokuTable semantic-name mismatch refuses",
            function () {
                var original = serializeIdentity(kinsokuA);
                var forged, result, roundTrip;

                require(original.ok, "KinsokuTable serialization failed.");
                require(original.state.family === CoreIdentity.FAMILY_KINSOKU_TABLE,
                    "KinsokuTable serialized to wrong family.");
                allStates.push(original.state);
                require(!containsLiveHostObject(original.state, 0),
                    "KinsokuTable serialized state contains a live host object.");

                roundTrip = resolveIdentity(doc, original.state);
                require(roundTrip.ok && roundTrip.status === "RESOLVED",
                    "KinsokuTable exact state did not resolve.");

                forged = copyState(original.state);
                forged.name = "__NF_MISSING_KINSOKU_TABLE__";

                result = resolveIdentity(doc, forged);
                require(!result.ok, "forged KinsokuTable name unexpectedly resolved.");
                require(result.status === "IDENTITY_CONFLICT",
                    "expected IDENTITY_CONFLICT, got " + result.status);
            }
        );
        ...
        runCase(
            "IR14",
            "KinsokuTable stored-ID conflict refuses",
            function () {
                var original = serializeIdentity(kinsokuA);
                var forged, result;

                require(original.ok, "KinsokuTable serialization failed.");
                require(objectId(kinsokuA) !== objectId(kinsokuB),
                    "KinsokuTable fixture IDs are not distinct.");
                allStates.push(original.state);
                require(!containsLiveHostObject(original.state, 0),
                    "KinsokuTable serialized state contains a live host object.");

                forged = copyState(original.state);
                forged.id = objectId(kinsokuB);

                result = resolveIdentity(doc, forged);
                require(!result.ok, "forged KinsokuTable ID unexpectedly resolved.");
                require(result.status === "IDENTITY_CONFLICT",
                    "expected IDENTITY_CONFLICT, got " + result.status);
            }
        );
        ...
        runCase(
            "IR15",
            "MojikumiTable semantic-name mismatch refuses",
            function () {
                var original = serializeIdentity(mojikumiA);
                var forged, result, roundTrip;

                require(original.ok, "MojikumiTable serialization failed.");
                require(original.state.family === CoreIdentity.FAMILY_MOJIKUMI_TABLE,
                    "MojikumiTable serialized to wrong family.");
                allStates.push(original.state);
                require(!containsLiveHostObject(original.state, 0),
                    "MojikumiTable serialized state contains a live host object.");

                roundTrip = resolveIdentity(doc, original.state);
                require(roundTrip.ok && roundTrip.status === "RESOLVED",
                    "MojikumiTable exact state did not resolve.");

                forged = copyState(original.state);
                forged.name = "__NF_MISSING_MOJIKUMI_TABLE__";

                result = resolveIdentity(doc, forged);
                require(!result.ok, "forged MojikumiTable name unexpectedly resolved.");
                require(result.status === "IDENTITY_CONFLICT",
                    "expected IDENTITY_CONFLICT, got " + result.status);
            }
        );
        ...
        runCase(
            "IR16",
            "MojikumiTable stored-ID conflict refuses",
            function () {
                var original = serializeIdentity(mojikumiA);
                var forged, result;

                require(original.ok, "MojikumiTable serialization failed.");
                require(objectId(mojikumiA) !== objectId(mojikumiB),
                    "MojikumiTable fixture IDs are not distinct.");
                allStates.push(original.state);
                require(!containsLiveHostObject(original.state, 0),
                    "MojikumiTable serialized state contains a live host object.");

                forged = copyState(original.state);
                forged.id = objectId(mojikumiB);

                result = resolveIdentity(doc, forged);
                require(!result.ok, "forged MojikumiTable ID unexpectedly resolved.");
                require(result.status === "IDENTITY_CONFLICT",
                    "expected IDENTITY_CONFLICT, got " + result.status);
            }
        );
